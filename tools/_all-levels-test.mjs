 ...
 */
function playThrough(level, themeId) {
...
// Each level is played on its own default theme and again on the theme with the
// tightest symbol supply, so a level is never declared playable on the strength
// of one lucky deal.
const playFaults = [];
const starTotals = { 1: 0, 2: 0, 3: 0 };
for (const level of LEVELS) {
  for (const themeId of [level.theme, THEME_IDS[level.id % THEME_IDS.length]]) {
    const r = playThrough(level, themeId);
    const cards = level.rows * level.cols;
    const why = [];
    ...
    if (why.length) playFaults.push(`L${level.id}/${themeId}: ${why.join(', ')}`);
    if (r.ended && r.ended.stars) starTotals[r.ended.stars] = (starTotals[r.ended.stars] || 0) + 1;
  }
}
ok(playFaults.length === 0,
  `all ${LEVELS.length} levels cleared on two themes each — ${LEVELS.length * 2} rounds, 0 faults${playFaults.length ? ' — ' + playFaults.slice(0, 5).join(' | ') : ''}`);
